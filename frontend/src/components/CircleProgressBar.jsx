...
const CircleProgressBar = ({ title, percentage }) => {
  const { languageSK, darkMode } = useThemeContext();

  return (
    <Wrapper percentage={percentage} darkMode={darkMode}>
      {' '}
      <div className="bar">
        <svg>
      ...
      <h3>{languageSK ? title[1] : title[0]}</h3>
    </Wrapper>
  );
};
...
const Wrapper = styled.div`
  ...
  .bar {
    ...
    box-shadow: ${(props) =>
      props.darkMode
        ? 'none'
        : `-3px -3px 7px #ffffffb2, 4px 4px 5px rgba(94, 104, 121, 0.945)`};
    overflow: hidden;
    margin: 1rem;
  }
  .bar::after {
